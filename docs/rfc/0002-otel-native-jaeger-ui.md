...
#### 1.3 Create Facade Selectors ✅ (goal met by a different design)
  ```typescript
...
#### 1.4 Create React Hooks ✅ (goal met by a different design)
  ```typescript
...
- [x] Implement `fetchTraceSummaries(query)` in `JaegerClient` using the `/api/v3/trace-summaries` endpoint.
  - Uses camelCase query params (e.g. `query.serviceName`) matching the OpenAPI spec and backend PR #8625.
  - Converts microsecond epoch timestamps to ISO 8601 for the API.
  - Returns `TraceSummary[]` with BigInt-safe nanosecond→microsecond conversion.
  - Zod runtime validation via `TraceSummariesResponseSchema` (permissive: only `traceId` required).
  - Schemas generated from `jaeger-idl` OpenAPI spec via `openapi-zod-client`; enriched in `schemas.ts`.
- [x] Map Search UI parameters (`SearchQuery`) to API query parameters in `client.ts`.
- [x] Create `useSearchTraces(query)` hook (`src/hooks/useTraceDiscovery.ts`) with React Query.
- [x] Update `SearchTracePage` to use `useSearchTraces` instead of Redux/JaegerAPI.
- **Verification**: Searching for traces from the UI returns results from `/api/v3/trace-summaries`.
...
**Status**: ✅ Complete (2026-05-25) - Runtime validation with Zod active for services, operations, and trace summaries endpoints.
...
- [x] Add `generate:api-types` script to `package.json` (root) to regenerate `src/api/v3/generated-client.ts` from the OpenAPI spec:
```json
{
  "scripts": {
...
`scripts/postprocess-schemas.cjs` adds a copyright header and strips the unused Zodios client code that the generator emits alongside the Zod schemas.

Schema aliasing (stable export names like `ServicesResponseSchema`) lives in `src/api/v3/schemas.ts`, not in the generated file.

**When to Regenerate**:
- When `jaeger-idl` OpenAPI spec changes (new fields, required annotations, etc.)
